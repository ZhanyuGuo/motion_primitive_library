...
namespace MPL
{
  ...
  public:
    ...
    bool plan(const Coord &start, const Coord &goal)
    {
      ...
      std::unique_ptr<MPL::GraphSearch<Dim, Coord>> planner_ptr(new MPL::GraphSearch<Dim, Coord>(planner_verbose_));
      ...
      else
      {
        ...
        {
          if (planner_verbose_)
            printf(ANSI_COLOR_CYAN "[PlannerBase] reset planner state space!" ANSI_COLOR_RESET "\n");
          ss_ptr_.reset(new MPL::StateSpace<Dim, Coord>(epsilon_));
        }
      }
