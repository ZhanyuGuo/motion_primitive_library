  ...
  public:
                    ...
                    Trajectory<Dim> &traj, int max_expand = -1)
    {
      ...
      while (true)
      {
        ...
        // 重点在计算后继节点(初值[2s], dt[1])和g(网络拟合或者离散化计算，先直接使用欧式距离), h(先使用欧式距离)
        // 碰撞检测的基元再生成(先不进行)
        ...
        {
          printf(ANSI_COLOR_RED "MaxExpandStep [%d] Reached!" ANSI_COLOR_RESET "\n", max_expand);
        ...
        if (ss_ptr->pq_.empty())
        {
          printf(ANSI_COLOR_RED "Priority queue is empty!" ANSI_COLOR_RESET "\n");
          return std::numeric_limits<decimal_t>::infinity();
        }
      }
      ...
      if (verbose_)
      {
        ...
        printf(ANSI_COLOR_GREEN "goalNode fval: %f, g: %f!" ANSI_COLOR_RESET "\n", fval, currNode_ptr->g);
        printf(ANSI_COLOR_GREEN "Expand [%d] nodes!" ANSI_COLOR_RESET "\n", expand_iteration);
      }

      if (ENV->is_goal(currNode_ptr->coord))
      {
        if (verbose_)
          printf(ANSI_COLOR_GREEN "Reached Goal!" ANSI_COLOR_RESET "\n");
      }
                      ...
                      Trajectory<Dim> &traj, int max_expand = -1)
    {
      ...
      {
        if (verbose_)
          printf(ANSI_COLOR_GREEN "Start is inside goal region!" ANSI_COLOR_RESET "\n");
        return 0;
      }
      ...
      if (!ss_ptr->best_child_.empty() && ENV->is_goal(ss_ptr->best_child_.back()->coord))
      {
        ...
        if (verbose_)
        {
          printf(ANSI_COLOR_GREEN "Use existing goal!" ANSI_COLOR_RESET "\n");
          printf(ANSI_COLOR_GREEN "goalNode fval: %f, g: %f, rhs: %f!" ANSI_COLOR_RESET "\n",
      ...
      if (verbose_)
      {
        if (ENV->is_goal(goalNode_ptr->coord))
          ...
          printf(ANSI_COLOR_RED "Terminated for unknown reason!" ANSI_COLOR_RESET "\n");
      }
