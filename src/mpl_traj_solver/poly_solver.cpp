...
template <int Dim>
PolySolver<Dim>::PolySolver(unsigned int smooth_derivative_order, unsigned int minimize_derivative, bool debug)
    : N_(2 * (smooth_derivative_order + 1)), R_(minimize_derivative), debug_(debug)
{
...
template <int Dim>
bool PolySolver<Dim>::solve(const vec_E<Waypoint<Dim>> &waypoints, const std::vector<decimal_t> &dts)
{
  ptraj_->clear();
      ...
      for (unsigned int r = 0; r < N_; r++)
      {
        if (r >= R_ && n >= R_)
        {
          ...
          for (unsigned int m = 0; m < R_; m++)
            val *= (r - m) * (n - m);
          Q(i * N_ + r, i * N_ + n) = val * power(seg_time, r + n - 2 * R_ + 1) / (r + n - 2 * R_ + 1);
        }
      }
    }
  }
  ...
  MatDf Rpp = R.block(num_fixed_derivatives, num_fixed_derivatives, num_free_derivatives, num_free_derivatives);
  MatDf Rpf = R.block(num_fixed_derivatives, 0, num_free_derivatives, num_fixed_derivatives);
  ...
  for (unsigned int i = 0; i < num_segments; i++)
  {
    const MatDNf<Dim> p = A.block(i * N_, i * N_, N_, N_).partialPivLu().solve(d.block(i * N_, 0, N_, Dim));
    if (debug_)
      std::cout << "p:\n"
                << p << std::endl;
    ptraj_->addCoeff(p);
